...
################################################################################
# Second Model
# Ensemble: Matrix Factorization and LightGBM  
################################################################################
...
set.seed(123)
# Load Data
avazu <- read_csv(local_file, show_col_types = FALSE)

# Advanced Feature Engineering (Combining both models)

avazu <- avazu %>%
  mutate(
    # Unified Placement (LightGBM)
    placement = ifelse(site_id == "85f751fd", app_id, site_id),
    hour_num = as.integer(substr(as.character(hour), 7, 8)),
    
    # Interactions (FM)
    sh_key = paste(site_id, hour_num),
    sb_key = paste(site_id, banner_pos),
    
    # User Proxy (Device + Ad ID)
    user_ad_proxy = paste(device_model, C14, sep = "_")
  ) %>%
  # Ad Fatigue 
  group_by(hour, C14) %>%
  mutate(ad_hourly_cnt = log1p(n())) %>% 
...
# Train/Test Split
train_idx <- sample(1:nrow(avazu), 0.8 * nrow(avazu))
train <- avazu[train_idx, ]
test  <- avazu[-train_idx, ]

# K-Fold Target
target_encode_kf <- function(train_df, test_df, col, k = 5) {
  train_df$fold <- sample(1:k, nrow(train_df), replace = TRUE)
  new_col <- paste0(col, "_te")
  g_mean <- mean(train_df$click)
  for (i in 1:k) {
    means <- train_df[train_df$fold != i, ] %>%
      group_by(!!sym(col)) %>%
      summarise(m = (sum(click) + 25 * g_mean) / (n() + 25))
    train_df[[new_col]][train_df$fold == i] <- left_join(train_df[train_df$fold == i, ], means, by = col)$m
  }
  test_means <- train_df %>% group_by(!!sym(col)) %>% summarise(m = (sum(click) + 25 * g_mean) / (n() + 25))
  test_df[[new_col]] <- left_join(test_df, test_means, by = col)$m
  train_df[[new_col]][is.na(train_df[[new_col]])] <- g_mean
  test_df[[new_col]][is.na(test_df[[new_col]])] <- g_mean
  return(list(train = train_df, test = test_df))}

# Encoding
for (f in c("placement", "device_model", "C14", "C17", "sh_key", "sb_key")) {
  res <- target_encode_kf(train, test, f)
  train <- res$train; test <- res$test}

# Latent Interaction from Model 1 & 2 - Matrix Factorization
u_map <- unique(train$device_model)
p_map <- unique(train$placement)
map_id <- function(x, ref) { match(x, ref, nomatch = length(ref) + 1) }

train_mf_data <- data_memory(user_index = map_id(train$device_model, u_map),
                             item_index = map_id(train$placement, p_map),
                             rating = train$click, index1 = TRUE)

r <- Reco()
r$train(train_mf_data, opts = list(dim = 50, niter = 100, lrate = 0.1, verbose = FALSE))

train$mf_score <- r$predict(train_mf_data, out_memory())
test_mf_data <- data_memory(user_index = map_id(test$device_model, u_map),
                            item_index = map_id(test$placement, p_map), index1 = TRUE)
test$mf_score <- r$predict(test_mf_data, out_memory())

# LightGBM
# All engineered features + FM score
features <- c("mf_score", "placement_te", "device_model_te", "C14_te", "C17_te", 
              "sh_key_te", "sb_key_te", "ad_hourly_cnt", "hour_num", "banner_pos")

dtrain <- lgb.Dataset(data = as.matrix(train[, features]), label = train$click)
...
lgb_model <- lgb.train(params, dtrain, nrounds = 500, verbose = -1)

# Evaluation
final_prob <- predict(lgb_model, as.matrix(test[, features]))
final_prob <- pmin(pmax(final_prob, 1e-15), 1 - 1e-15)

auc_val <- performance(prediction(final_prob, test$click), "auc")@y.values[[1]]
logloss_val <- -mean(test$click * log(final_prob) + (1 - test$click) * log(1 - final_prob))

cat("AUC:", round(auc_val, 5), "\n")
cat("LogLoss:", round(logloss_val, 5), "\n")

# Check the training parameters: 63 leaves decision tree
print(lgb_model$params$num_leaves)
